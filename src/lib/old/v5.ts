...
const isSignal = <T>(value: unknown): value is Signal<T> => {
  return (
    isObject(value) &&
    hasOwnPropertyType(value, "read", isFunction) &&
    hasOwnPropertyType(value, "write", isFunction) &&
    hasOwnPropertyType(value, "subscribe", isFunction) &&
    hasOwnPropertyType(value, "unsubscribe", isFunction)
  );
};
...
const Button = $<{ onClick: () => void; text: string }>((props) => {
  return button({
    onClick: () => {
      props.onClick();
    },
